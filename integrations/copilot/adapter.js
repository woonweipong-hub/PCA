...
function main() {
  ...
  if (!parsed.command || ['prepare', 'assess'].indexOf(parsed.command) === -1) {
    fail('Usage: node integrations/copilot/adapter.js <prepare|assess> <discuss|verify> ...');
  }
  ...
  if (parsed.command === 'prepare') {
    ...
    process.stdout.write(`${JSON.stringify({ platform: 'copilot', kind: 'prepare', pca: session }, null, 2)}\n`);
    return;
  }
  ...
  process.stdout.write(`${JSON.stringify({ platform: 'copilot', kind: 'assess', pca: result }, null, 2)}\n`);
}
